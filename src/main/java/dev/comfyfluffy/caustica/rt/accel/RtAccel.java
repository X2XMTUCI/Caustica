...
public final class RtAccel {
    ...
    /**
     * Keep a completed terrain BLAS in its original allocation and discard only its compacted-size
     * query. This is used for latency-sensitive block edits that deliberately skip the compact-copy.
     */
    public static void discardTerrainCompactionQuery(PreparedBlas source) {
        long queryPool = source.accel.compactionQueryPool;
        if (queryPool != 0L) {
            VK10.vkDestroyQueryPool(source.accel.vk, queryPool, null);
            source.accel.compactionQueryPool = 0L;
        }
    }
